...
class RiceDataLoader(Dataset):
    ...
    def __init__(
        ...
        self.image_paths = list(self.dataset_path.glob("*/*.png")) + list(
            self.dataset_path.glob("*/*.jpg")
        )
        ...
        self.class_to_idx = {
            class_name: idx for idx, class_name in enumerate(set(self.labels))
        }
    ...
    def extract_and_sample_data(self):
        ...
        for class_folder in tqdm(dataset_path.iterdir(), desc="Sampling classes"):
            if class_folder.is_dir():
                images = list(class_folder.glob("*.png")) + list(
                    class_folder.glob("*.jpg")
                )
                sampled_images = random.sample(
                    images, max(1, int(len(images) * self.sample_ratio))
                )
