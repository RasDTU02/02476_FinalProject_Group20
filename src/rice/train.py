...
# python src/rice/train.py
# (remember to set cd to the root of the project)
...
def initialize_wandb(cfg: DictConfig):
    ...
    wandb.init(
        ...
        # The first argument is the name of the run, the second one is if the
        # first one can not be used
...
def load_data(cfg: DictConfig):
    ...
    for class_idx, class_dir in enumerate(class_dirs):
        ...
        num_samples = min(
            ...
            int(len(images) * cfg.training_conf.sample_ratio),
        )
        # Brug mindste værdi mellem max_images og sample_ratio*antal billeder
