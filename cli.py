...
def train(
    root_dir: str = typer.Option(
        None, help="Path to the dataset root directory"
    ),
    resize: list[int] = typer.Option(
        None, help="Resize dimensions for the images (width, height)"
    ),
    subset: int = typer.Option(
        None,
        help="Samples for evenly distributed across classes)",
    ),
    ...
    using default configurations with CLI overrides."""
    ...
    if resize is not None:
        command.extend(
            [f"data.resize[0]={resize[0]}", f"data.resize[1]={resize[1]}"]
        )
